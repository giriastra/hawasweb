...
<div class="col-xl-12 col-lg-12" style="float:left; padding: ">
        ...
        <div class="mesgs">
          <div class="msg_history">
            ...
            <?php foreach ($this->model_global->getDataGlobal('tb_complaint_detail','id_complaint',$this->uri->segment(2))->result() as $rowChat) {
              if ($rowChat->id_petugas>0) {
            ?>
                <div class="outgoing_msg">
                  <div class="outgoing_msg_img"> <img src="<?=base_url()?>assets/img/user.png" alt="sunil"> </div>
                  <div class="sent_msg">
                    <p><?=$rowChat->message?><span class="time_date_sent"> <?=$this->model_global->tgl_indo($rowChat->date)." ".date("H:i", strtotime($rowChat->date)) ?></span></p>
                  </div>
                </div>
              <?php
              } else {
                 ...
                 <div class="incoming_msg">
                  ...
                  <div class="received_msg">
                    <div class="received_withd_msg">
                      <p>
                        <b style="color: #fff">
                          <?=@$this->model_global->getDataGlobal('tb_user','id_user',$rowChat->id_user)->row()->name?> ~
                        </b>
                        <br>
                        <?=$rowChat->message?><span class="time_date"> <?=$this->model_global->tgl_indo($rowChat->date)." ".date("H:i", strtotime($rowChat->date)) ?></span>
                      </p>
                    </div>
                  </div>
                </div>
            ...
            <div class="input_msg_write">
              <?php
                if ($_GET['status']!='OPEN' &&  $_GET['id_petugas'] != $this->session->userdata('id_user')) {
                    echo "<div style='padding:20px;color:red;'><center><b>*Anda tidak memiliki otoritas untuk melakukan percakapan di pengaduan ini!</b></div></div>";
                } else if ($_GET['status'] == 'CANCEL' || $_GET['status']  == 'REJECT') {
                    echo "<div style='padding:20px;color:red;'><center><b>*Anda tidak memiliki otoritas untuk melakukan percakapan di pengaduan ini!</b></div></div>";
                } else{
              ?>
    ...
    $("#pageloader").fadeIn();
      setTimeout(function() {
        if ('<?=$_GET['status']?>'!='OPEN' && '<?=$_GET['id_petugas']?>'!='<?=$this->session->userdata('id_user')?>') {
            ...
            datasend.append('id_complaint',id_forum);
            datasend.append('pesan',$('#isi_pesan').val());
            $.ajax({
                url: '<?=base_url()?>Utility/SendComplaint',
                method: 'POST',
                contentType: false,
                processData: false,
                data: datasend,
